...
OptoPupil is a browser-based computer vision application designed to explore quantitative pupillary light reflex (PLR) screening using a standard device camera and display.

The project aims to reduce dependency on specialized pupillometry hardware by establishing a camera-based computer vision pipeline capable of detecting and tracking ocular landmarks in real time. The current implementation represents the first computer vision milestone of the system, focusing on eye and iris localization, tracking quality assessment, geometric measurements, and real-time visualization.
...
https://optopupil.vercel.app/

Kaggle Notebook

https://www.kaggle.com/code/jyotish1628/notebook64ebcbbf14/notebook

Kaggle Real Dataset

https://www.kaggle.com/datasets/itguides/pupil-eye-and-iris-segmentation
...
The current implementation establishes the computer vision infrastructure required for future quantitative pupil-response analysis.
...
The vision pipeline operates locally within the browser and is designed for real-time processing using WebAssembly and GPU acceleration where supported.
...
The camera stream is combined with a canvas-based visualization layer that displays detected ocular landmarks and tracking information.
...
OptoPupil uses a unified application architecture for different device classes, with interfaces designed for both desktop and mobile viewport configurations.
...
Captures frames from the device camera.

Passes frames to the MediaPipe Face Landmarker.

Retrieves facial and ocular landmarks.

Extracts eye contours and iris geometry.

Computes normalized ocular measurements.

Evaluates tracking quality.

Renders the results through the canvas visualization layer.
